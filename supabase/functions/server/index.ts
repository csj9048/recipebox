...
app.post("/server/analyze-recipe-image", async (c) => {
  try {
...
{
  "title": "요리 이름",
  "body_text": "재료 목록과 만드는 방법만 포함. 반드시 [재료]와 [방법] 섹션으로 구조화하고, 재료는 각 줄 앞에 하이픈(-)을 붙여서 리스트 형태로 표시해주세요. 서론, 설명, 꾸밈말은 모두 제외하고 순수하게 [재료]와 [방법] 내용만 추출해주세요.",
  "ingredients": ["재료1", "재료2", ...],
...
2. body_text에는 요리에 대한 설명이나 감상, 추천 문구 등은 절대 포함하지 말고, 오직 재료 목록과 만드는 방법(조리 순서)만 추출해주세요.
...
app.post("/server/delete-user", async (c) => {
  try {
    ...
    // Delete user using admin client (service role key has admin privileges)
